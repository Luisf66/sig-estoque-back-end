// tests/unit/services/purchase/create-purchase.test.ts

import { describe, it, expect, beforeEach, vi } from 'vitest';
import { CreatePurchaseService } from "../../../src/services/purchase/create-purchase";
...
import type { Purchase, Item, Product } from "@prisma/client";
...
describe("CreatePurchaseService", () => {
  ...
  describe("handle", () => {
    ...
    const mockUpdatedPurchase: Purchase = {
      ...mockPurchase,
      sub_total: 333.75, // (10 * 25.50) + (5 * 15.75) = 255.00 + 78.75 = 333.75
    ...
    it("should create purchase successfully with multiple items", async () => {
      // Arrange
      vi.mocked(mockPurchaseRepository.create).mockResolvedValue(mockPurchase);
      ...
      vi.mocked(mockPurchaseRepository.updateSubTotal).mockResolvedValue(mockUpdatedPurchase);

      // Act
      const result = await createPurchaseService.handle(mockRequest);

      // Assert
      expect(mockPurchaseRepository.create).toHaveBeenCalledWith({
        nf_number: mockRequest.nf_number,
        supplier: { connect: { id: mockRequest.supplierId } },
        user: { connect: { id: mockRequest.userId } },
      });

      expect(mockProductRepository.findManyByIds).toHaveBeenCalledWith(["product-1", "product-2"]);

      expect(mockProductRepository.increaseStock).toHaveBeenCalledWith("product-1", 10);
      expect(mockProductRepository.increaseStock).toHaveBeenCalledWith("product-2", 5);

      expect(mockItemRepository.create).toHaveBeenCalledTimes(2);
      expect(mockItemRepository.create).toHaveBeenNthCalledWith(1, {
        purchase: { connect: { id: mockPurchase.id } },
        quantity: 10,
        value: 25.50,
      ...
      expect(mockPurchaseRepository.updateSubTotal).toHaveBeenCalledWith("purchase-123", 333.75);

      expect(result.newPurchase).toEqual(mockUpdatedPurchase);
    ...
    it("should throw error when product not found", async () => {
      ...
      // Act & Assert
      await expect(createPurchaseService.handle(mockRequest))
        .rejects
        .toThrow("Product not found");

      expect(mockPurchaseRepository.create).toHaveBeenCalled();
      expect(mockProductRepository.findManyByIds).toHaveBeenCalledWith(["product-1", "product-2"]);
      expect(mockProductRepository.increaseStock).not.toHaveBeenCalled();
      expect(mockItemRepository.create).not.toHaveBeenCalled();
      expect(mockPurchaseRepository.updateSubTotal).not.toHaveBeenCalled();
    });
    ...
    it("should throw error when product is inactive", async () => {
      ...
      await expect(createPurchaseService.handle(mockRequest))
        .rejects
        .toThrow(`Product ${mockInactiveProduct.name} does not exist or is inactive`);
    ...
    it("should propagate errors from purchase repository during creation", async () => {
      // Arrange
      const repositoryError = new Error("Purchase creation failed");
      vi.mocked(mockPurchaseRepository.create).mockRejectedValue(repositoryError);

      // Act & Assert
      await expect(createPurchaseService.handle(mockRequest))
        .rejects
        .toThrow("Purchase creation failed");
    ...
    it("should propagate errors from product repository during stock increase", async () => {
      // Arrange
      vi.mocked(mockPurchaseRepository.create).mockResolvedValue(mockPurchase);
      vi.mocked(mockProductRepository.findManyByIds).mockResolvedValue(mockActiveProducts as Product[]);
      const repositoryError = new Error("Stock increase failed");
      vi.mocked(mockProductRepository.increaseStock).mockRejectedValue(repositoryError);

      // Act & Assert
      await expect(createPurchaseService.handle(mockRequest))
        .rejects
        .toThrow("Stock increase failed");

      expect(mockPurchaseRepository.create).toHaveBeenCalled();
      expect(mockProductRepository.findManyByIds).toHaveBeenCalled();
      expect(mockProductRepository.increaseStock).toHaveBeenCalled();
      expect(mockItemRepository.create).not.toHaveBeenCalled();
    });

    it("should handle purchase with single item", async () => {
      // Arrange
      ...
      const updatedPurchaseWithSingleItem: Purchase = {
        ...mockPurchase,
        sub_total: 255.00, // 10 * 25.50
        updated_at: new Date('2023-01-02'),
      };
      ...
      vi.mocked(mockPurchaseRepository.updateSubTotal).mockResolvedValue(updatedPurchaseWithSingleItem);

      // Act
      const result = await createPurchaseService.handle(singleItemRequest);
      ...
      expect(mockPurchaseRepository.updateSubTotal).toHaveBeenCalledWith("purchase-123", 255.00);
      expect(result.newPurchase.sub_total).toBe(255.00);
      expect(result.items).toEqual(singleItem);
    });

    it("should handle purchase with zero quantity items", async () => {
      // Arrange
      ...
      const updatedPurchaseWithZeroTotal: Purchase = {
        ...mockPurchase,
        sub_total: 0,
        updated_at: new Date('2023-01-02'),
      };
      ...
      vi.mocked(mockPurchaseRepository.updateSubTotal).mockResolvedValue(updatedPurchaseWithZeroTotal);

      // Act
      const result = await createPurchaseService.handle(zeroQuantityRequest);

      // Assert
      expect(mockProductRepository.increaseStock).toHaveBeenCalledWith("product-1", 0);
      expect(mockPurchaseRepository.updateSubTotal).toHaveBeenCalledWith("purchase-123", 0);
      expect(result.newPurchase.sub_total).toBe(0);
    });

    it("should handle purchase with zero value items", async () => {
      // Arrange
      ...
      // Act
      const result = await createPurchaseService.handle(zeroValueRequest);
    ...
    it("should calculate correct subtotal for multiple items", async () => {
      ...
      vi.mocked(mockPurchaseRepository.updateSubTotal).mockResolvedValue({
        ...mockPurchase,
        sub_total: expectedSubTotal,
      });

      // Act
      const result = await createPurchaseService.handle(complexItemsRequest);

      // Assert
      expect(mockPurchaseRepository.updateSubTotal).toHaveBeenCalledWith("purchase-123", expectedSubTotal);
      expect(result.newPurchase.sub_total).toBe(expectedSubTotal);
    });
    ...
    it("should handle empty items array", async () => {
      ...
      // Act
      const result = await createPurchaseService.handle(emptyItemsRequest);
